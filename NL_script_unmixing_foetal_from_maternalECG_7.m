...
time=X(:,1)' ; ECGdata=X(:,2:7)';

B=jadeR(ECGdata,4); % deriving the unmixing-matrix
...
figure,plot(time,zscore(Sources(1,:)),time,zscore(Sources(3,:))+6),grid,xlabel('time'),legend('1st','3d')
...
[sources] = fastica (ECGdata,'numOfIC', 2);  
%[icasig] = fastica (ECGdata, 'lastEig', 4, 'numOfIC', 3);
